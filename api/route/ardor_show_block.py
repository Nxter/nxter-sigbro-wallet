...
class ardor_show_block:
  def on_get(self, req, resp, block, lang='en'):
    ...
    if 'data' in blk:
      ...
      blkTotalFee = _a['totalFeeFQT']
      blkTotalFee_b = utils.beauty_int( utils.nqt2ardr(  blkTotalFee ) )
      
      blkTimestamp = _a['timestamp']
      blkTimestamp_b = utils.timestamp2ardortime( blkTimestamp )

      if 'nextBlock' in _a:
        blkNext = _a['nextBlock']
        blkNext_url = """<a href='/explorer/ablock-%s' target=_self>%s</a> """ % ( blkNext, blkNext ) 
      else:
        blkNext_url = 'None'

      if 'previousBlock' in _a:
        blkPrev = _a['previousBlock']
        blkPrev_url = """<a href='/explorer/ablock-%s' target=_self>%s</a> """ % ( blkPrev, blkPrev ) 
      else:
        blkPrev_url = 'None'
    ...
    <div class="nb row">
      <div class="nb col-xs-12 col-sm-12 col-md-12">
        ...
        <table class="nb table table-striped">
          ...
          <tr><td>{{Total Fee}}</td><td>%s ARDR</td></tr> 
      ...
      """ % ( block, blkHeight, blkGen_url, blkTotalFee_b, blkTimestamp_b, blkNext_url, blkPrev_url, blkTransactionNumbers )
    ...
    <div class="nb container">
      <div class="nb row">
        ...
        for i in tr_list:
          ...
          tr_url = """<a href='/explorer/fxt-%s' target=_blank>%s</a>""" % ( tr['tr'], tr['tr'] )
          tr_time = utils.timestamp2ardortime( tr['timestamp'] )
